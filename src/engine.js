...
const WORLD_DOC = {
  ...
  map: {
    ...
    nodes: {
      beach:      { name:"Shingle Beach", desc:"Where the sea spat you out. The Meridian lies broken on the reef, a hundred yards out. The jungle waits inland; the cove and your camp sit along the shore.", exits:{ west:"tide_pools", north:"jungle", east:"wreck_shore", up:"cliff", south:"cove", south_east:"camp" }, items:["driftwood","debris"] },
...
// Apply structured lore entities (portable → location items). Rejects meta strings.
function applyEntities(entities){
  ...
  for(const e of (entities||[])){
    ...
    if(portable && !n.items.includes(id) && !hasItem(id)) n.items.push(id);
  }
}
...
function matchItem(here, tgt){
  const t=singularize(String(tgt).toLowerCase().replace(/_/g," ").trim());
  return here.find(i=>{
    if(hasItem(i)) return false;
    const n=itemName(i).toLowerCase();
    const id=i.replace(/_/g," ");
    return i===t || id===t || singularize(n)===t || n.includes(t) || t.includes(singularize(n)) ||
      i.includes(t.replace(/ /g,"_")) || singularize(id)===t;
  });
}
function doTake(t){
  if(!t) return {ok:false, text:"Take what?"};
  // multi-take: "fig and bamboo" / "fig, bamboo"
  const parts = String(t).split(/\s+and\s+|,\s*/).map(x=>x.trim()).filter(Boolean);
  if(parts.length>1){
    const got=[], miss=[];
...
{"answer":"one vivid factual sentence answering the player","facts":["optional short lore facts"],"entities":[{"id":"snake_case","name":"display name","portable":true|false,"desc":"one short line"}]}
Rules: no cold/volcanic life, no magic. Entities only if visible/usable at the current place. portable=true only for things the player could pick up. Keep JSON complete and short.
...
RECENT:\n${recent||"(none)"}`;
  ...
  return cleaned ? {answer:cleaned, facts:[], entities:[]} : null;
}
...
export function createGame(opts = {}) {
  ...
  async function genStep(pending) {
    ...
    if (pending.type === "lore") {
      ...
      if (!out || !(out.answer || (out.entities && out.entities.length) || (out.facts && out.facts.length))) {
      ...
      return {
        ...
        answer: out.answer, facts: out.facts, entities: out.entities
      };
    }
